...
<div class="space-y-6">
	...
	{#if data.blocks.length === 0}
		<p class="text-surface-500 py-12 text-center">No service blocks found.</p>
	{:else}
		<div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
			{#each data.blocks as block}
				<a href="/blocks/{block.id}" class="preset-outlined-surface-50 block rounded-lg p-6 transition-shadow hover:shadow-lg">
					...
					{#if block.termName}
						<p class="text-surface-500 text-sm">{block.termName}</p>
					{/if}
					{#if block.seriesTitle}
						<p class="text-primary-600 mt-1 text-sm">Series: {block.seriesTitle}</p>
					{/if}
					{#if block.startDate && block.endDate}
						<p class="text-surface-9000 mt-2 text-xs">{formatDate(block.startDate)} – {formatDate(block.endDate)}</p>
					{/if}
				</a>
			{/each}
		</div>
