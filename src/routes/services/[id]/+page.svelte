<script lang="ts">
	...
	let activeTab = $state(form?.tab ?? 'overview');
...
<div class="space-y-6">
	<div class="flex items-center gap-4">
		<a href="/services" class="text-surface-400 hover:text-surface-200">&larr; Back</a>
	...
	{#if form?.success}
		<div class="rounded bg-green-900 p-4 text-green-200">
	...
	<div class="border-surface-700 flex gap-1 border-b">
		<button class="px-4 py-2 text-sm {activeTab === 'overview' ? 'border-primary-500 text-primary-400 border-b-2' : 'text-surface-400 hover:text-surface-200'}" onclick={() => activeTab = 'overview'}>Overview</button>
		<button class="px-4 py-2 text-sm {activeTab === 'roles' ? 'border-primary-500 text-primary-400 border-b-2' : 'text-surface-400 hover:text-surface-200'}" onclick={() => activeTab = 'roles'}>Roles ({data.service.roles.length})</button>
		<button class="px-4 py-2 text-sm {activeTab === 'hospitality' ? 'border-primary-500 text-primary-400 border-b-2' : 'text-surface-400 hover:text-surface-200'}" onclick={() => activeTab = 'hospitality'}>Hospitality ({externalRoles.length})</button>
		<button class="px-4 py-2 text-sm {activeTab === 'readings' ? 'border-primary-500 text-primary-400 border-b-2' : 'text-surface-400 hover:text-surface-200'}" onclick={() => activeTab = 'readings'}>Readings ({data.service.readings.length})</button>
		<button class="px-4 py-2 text-sm {activeTab === 'music' ? 'border-primary-500 text-primary-400 border-b-2' : 'text-surface-400 hover:text-surface-200'}" onclick={() => activeTab = 'music'}>Music ({data.service.music.length})</button>
	</div>
	...
	{#if activeTab === 'overview'}
		<form method="POST" action="?/update" class="max-w-2xl space-y-6">
			<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
				<div>
					...
					<select id="serviceType" name="serviceType" required class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2">
				...
				<div>
					...
					<input type="date" id="date" name="date" required value={data.service.date} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2" />
				</div>
				<div>
					<label for="time" class="mb-1 block text-sm font-medium">Start Time</label>
					<input type="time" id="time" name="time" value={data.service.time ?? ''} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2" />
				</div>
				<div>
					<label for="endTime" class="mb-1 block text-sm font-medium">End Time</label>
					<input type="time" id="endTime" name="endTime" value={data.service.endTime ?? ''} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2" />
				</div>
			</div>

			<div>
				<label for="title" class="mb-1 block text-sm font-medium">Title</label>
				<input type="text" id="title" name="title" value={data.service.title ?? ''} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2" />
			</div>

			<div class="grid grid-cols-1 gap-4 md:grid-cols-3">
				<div>
					<label for="rite" class="mb-1 block text-sm font-medium">Rite</label>
					<select id="rite" name="rite" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2">
				...
				<div>
					...
					<select id="visibility" name="visibility" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2">
				...
				<div>
					...
					<input type="text" id="location" name="location" value={data.service.location ?? 'Chapel'} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2" />
				</div>
			</div>
			...
			<div class="grid grid-cols-1 gap-4 md:grid-cols-3">
				<div>
					...
					<input type="text" id="liturgicalDay" name="liturgicalDay" value={data.service.liturgicalDay ?? ''} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2" />
				</div>
				<div>
					<label for="liturgicalSeason" class="mb-1 block text-sm font-medium">Season</label>
					<select id="liturgicalSeason" name="liturgicalSeason" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2">
				...
				<div>
					...
					<select id="liturgicalColour" name="liturgicalColour" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2">
			...
			<div>
				...
				<select id="blockId" name="blockId" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2">
			...
			<div>
				...
				<textarea id="notes" name="notes" rows="3" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2">{data.service.notes ?? ''}</textarea>
			</div>

			<div>
				<label for="specialInstructions" class="mb-1 block text-sm font-medium">Special Instructions</label>
				<textarea id="specialInstructions" name="specialInstructions" rows="2" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2">{data.service.specialInstructions ?? ''}</textarea>
			</div>
		...
		<form method="POST" action="?/delete" class="mt-4">
			<button type="submit" class="rounded bg-red-900 px-6 py-2 text-red-200 hover:bg-red-800" onclick={(e) => { if (!confirm('Delete this service?')) e.preventDefault(); }}>Delete Service</button>
		</form>
	...
	{:else if activeTab === 'roles'}
		<div class="max-w-2xl">
			...
			{#if showAddRole}
				<form method="POST" action="?/addRole" class="border-surface-700 mb-6 space-y-4 rounded border p-4">
					<h3 class="font-medium">Add New Role</h3>
					<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
						<div>
							<label for="newRole" class="mb-1 block text-sm font-medium">Role *</label>
							<select id="newRole" name="role" required class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2">
						...
						<div>
							...
							<select id="newPerson" name="personId" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2">
					...
					<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
						<div>
							...
							<input type="text" id="newRoleLabel" name="roleLabel" placeholder="e.g. First Reader" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2" />
						</div>
						<div>
							<label for="newStatus" class="mb-1 block text-sm font-medium">Status</label>
							<select id="newStatus" name="invitationStatus" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2">
					...
					<div>
						...
						<input type="text" id="newRoleNotes" name="notes" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2" />
					</div>
			...
			{#if data.service.roles.length === 0}
				<p class="text-surface-400 py-8 text-center">No roles assigned yet.</p>
			{:else}
				<div class="space-y-3">
					{#each data.service.roles as role}
						<div class="border-surface-700 rounded border p-4">
							<div class="flex items-start justify-between">
								<div>
									<div class="flex items-center gap-2">
										<span class="font-medium">{RoleLabels[role.role as keyof typeof RoleLabels] ?? role.role}</span>
										{#if role.roleLabel}
											<span class="text-surface-400 text-sm">({role.roleLabel})</span>
										{/if}
									</div>
									<div class="text-surface-300 mt-1 text-sm">
										{#if role.personId}
											<a href="/people/{role.personId}" class="text-primary-400 hover:underline">{personDisplayName(role)}</a>
											{#if role.personEmail}
												<span class="text-surface-500"> &middot; {role.personEmail}</span>
											{/if}
										{:else}
											<span class="text-surface-500 italic">Unassigned</span>
										{/if}
									</div>
									{#if role.notes}
										<div class="text-surface-400 mt-1 text-xs">{role.notes}</div>
									{/if}
									{#if role.invitedAt}
										<div class="text-surface-500 mt-1 text-xs">Invited: {formatDate(role.invitedAt)}</div>
									{/if}
									{#if role.respondedAt}
										<div class="text-surface-500 text-xs">Responded: {formatDate(role.respondedAt)}</div>
									{/if}
								</div>
								<div class="flex items-center gap-2">
							...
							<div class="mt-3 flex flex-wrap items-center gap-2 border-t border-surface-700 pt-3">
								{#if role.invitationStatus === 'possibility'}
									<form method="POST" action="?/updateStatus" class="inline">
										<input type="hidden" name="roleId" value={role.id} />
										<input type="hidden" name="status" value="requested" />
										<button type="submit" class="rounded bg-blue-900 px-3 py-1 text-xs text-blue-200 hover:bg-blue-800">Mark Requested</button>
									</form>
								{/if}
								{#if role.invitationStatus === 'requested'}
									<form method="POST" action="?/updateStatus" class="inline">
										...
										<button type="submit" class="rounded bg-green-900 px-3 py-1 text-xs text-green-200 hover:bg-green-800">Accept</button>
									</form>
									<form method="POST" action="?/updateStatus" class="inline">
										<input type="hidden" name="roleId" value={role.id} />
										<input type="hidden" name="status" value="declined" />
										<button type="submit" class="rounded bg-red-900 px-3 py-1 text-xs text-red-200 hover:bg-red-800">Decline</button>
									</form>
								{/if}
								{#if role.invitationStatus === 'declined'}
									<form method="POST" action="?/updateStatus" class="inline">
										...
										<button type="submit" class="rounded bg-surface-700 px-3 py-1 text-xs text-surface-200 hover:bg-surface-600">Reset</button>
									</form>
								{/if}
								...
								<button class="text-surface-400 ml-auto text-xs hover:text-surface-200" onclick={() => editingRoleId = editingRoleId === role.id ? null : role.id}>
							...
							{#if editingRoleId === role.id}
								<form method="POST" action="?/updateRole" class="mt-3 space-y-3 border-t border-surface-700 pt-3">
									<input type="hidden" name="roleId" value={role.id} />
									<div class="grid grid-cols-1 gap-3 md:grid-cols-2">
										<div>
											<label for="editRole-{role.id}" class="mb-1 block text-xs font-medium">Role</label>
											<select id="editRole-{role.id}" name="role" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm">
												{#each Object.entries(RoleLabels) as [value, label]}
										...
										<div>
											...
											<select id="editPerson-{role.id}" name="personId" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm">
												<option value="">Unassigned</option>
												{#each data.people as person}
									...
									<div class="grid grid-cols-1 gap-3 md:grid-cols-2">
										<div>
											...
											<input type="text" id="editRoleLabel-{role.id}" name="roleLabel" value={role.roleLabel ?? ''} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm" />
										</div>
										<div>
											<label for="editNotes-{role.id}" class="mb-1 block text-xs font-medium">Notes</label>
											<input type="text" id="editNotes-{role.id}" name="notes" value={role.notes ?? ''} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm" />
										</div>
									</div>
	...
	{:else if activeTab === 'hospitality'}
		<div class="max-w-2xl">
			...
			{#if externalRoles.length === 0}
				<p class="text-surface-400 py-8 text-center">No external visitors assigned to this service. Add non-college-member roles first.</p>
			{:else}
				<div class="space-y-4">
						...
						<div class="border-surface-700 rounded border p-4">
							<div class="mb-3 flex items-center justify-between">
								<div>
									...
									<span class="text-surface-400 text-sm"> — {RoleLabels[role.role as keyof typeof RoleLabels] ?? role.role}</span>
								</div>
								<button class="text-primary-400 text-sm hover:underline" onclick={() => hospitalityRoleId = hospitalityRoleId === role.id ? null : role.id}>
							...
							{#if hosp}
								<div class="grid grid-cols-2 gap-2 text-sm md:grid-cols-4">
									<div>
										<span class="text-surface-400 text-xs">Accommodation</span>
										<div>{BookingStatusLabels[hosp.accommodationStatus ?? 'not_needed']}</div>
									</div>
									<div>
										<span class="text-surface-400 text-xs">Meals</span>
										<div>{BookingStatusLabels[hosp.mealStatus ?? 'not_needed']}</div>
									</div>
									<div>
										<span class="text-surface-400 text-xs">Parking</span>
										<div>{BookingStatusLabels[hosp.parkingStatus ?? 'not_needed']}</div>
									</div>
									<div>
										<span class="text-surface-400 text-xs">Expenses</span>
										<div>
											{BookingStatusLabels[hosp.expensesStatus ?? 'not_needed']}
											{#if hosp.expensesAmount}
												<span class="text-surface-300"> (&pound;{hosp.expensesAmount.toFixed(2)})</span>
											{/if}
										</div>
									</div>
								</div>
							{:else}
								<p class="text-surface-500 text-sm">No hospitality details recorded.</p>
							{/if}

							{#if hospitalityRoleId === role.id}
								<form method="POST" action="?/saveHospitality" class="mt-4 space-y-4 border-t border-surface-700 pt-4">
									<input type="hidden" name="roleId" value={role.id} />

									<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
										<div>
											<label for="accStatus-{role.id}" class="mb-1 block text-xs font-medium">Accommodation</label>
											<select id="accStatus-{role.id}" name="accommodationStatus" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm">
										...
										<div>
											...
											<input type="text" id="accNotes-{role.id}" name="accommodationNotes" value={hosp?.accommodationNotes ?? ''} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm" />
										</div>
									</div>
									<div>
										<label for="accDates-{role.id}" class="mb-1 block text-xs font-medium">Accommodation Dates</label>
										<input type="text" id="accDates-{role.id}" name="accommodationDates" placeholder="e.g. 2025-10-15, 2025-10-16" value={hosp?.accommodationDates ?? ''} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm" />
									</div>

									<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
										<div>
											<label for="mealStatus-{role.id}" class="mb-1 block text-xs font-medium">Meals</label>
											<select id="mealStatus-{role.id}" name="mealStatus" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm">
												{#each Object.entries(BookingStatusLabels) as [value, label]}
										...
										<div>
											...
											<input type="text" id="mealNotes-{role.id}" name="mealNotes" value={hosp?.mealNotes ?? ''} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm" />
										</div>
									</div>
									...
									<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
										<div>
											...
											<select id="parkStatus-{role.id}" name="parkingStatus" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm">
												{#each Object.entries(BookingStatusLabels) as [value, label]}
										...
										<div>
											...
											<input type="text" id="parkNotes-{role.id}" name="parkingNotes" value={hosp?.parkingNotes ?? ''} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm" />
										</div>
									</div>
									...
									<div class="grid grid-cols-1 gap-4 md:grid-cols-3">
										<div>
											...
											<select id="expStatus-{role.id}" name="expensesStatus" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm">
												{#each Object.entries(BookingStatusLabels) as [value, label]}
										...
										<div>
											...
											<input type="number" id="expAmount-{role.id}" name="expensesAmount" step="0.01" value={hosp?.expensesAmount ?? ''} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm" />
										</div>
										<div>
											<label for="expNotes-{role.id}" class="mb-1 block text-xs font-medium">Expenses Notes</label>
											<input type="text" id="expNotes-{role.id}" name="expensesNotes" value={hosp?.expensesNotes ?? ''} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm" />
										</div>
									</div>
		...
		<div class="max-w-2xl">
			...
			{#if data.lectionaryOccasion && data.suggestedReadings.length > 0 && data.service.readings.length === 0}
				<div class="mb-6 rounded border border-blue-800 bg-blue-950 p-4">
					<h3 class="mb-2 font-medium text-blue-200">Suggested Readings — {data.lectionaryOccasion.name}</h3>
			...
			{#if showAddReading}
				<form method="POST" action="?/addReading" class="border-surface-700 mb-6 space-y-4 rounded border p-4">
					<h3 class="font-medium">Add Reading</h3>
					<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
						<div>
							<label for="newReadingType" class="mb-1 block text-sm font-medium">Type *</label>
							<select id="newReadingType" name="readingType" required class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2">
						...
						<div>
							...
							<input type="text" id="newReference" name="reference" required placeholder="e.g. Isaiah 40.1–11" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2" />
						</div>
					</div>
					<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
						<div>
							<label for="newReader" class="mb-1 block text-sm font-medium">Reader</label>
							<select id="newReader" name="readerId" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2">
								<option value="">Unassigned</option>
								{#each data.people as person}
									<option value={person.id}>
										{person.title ? person.title + ' ' : ''}{person.firstName} {person.lastName}
						...
						<div>
							...
							<input type="number" id="newReadingSortOrder" name="sortOrder" value={data.service.readings.length + 1} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2" />
						</div>
					</div>
					<div>
						<label for="newReadingNotes" class="mb-1 block text-sm font-medium">Notes</label>
						<input type="text" id="newReadingNotes" name="notes" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2" />
					</div>
					<div class="flex items-center gap-2">
			...
			{#if data.service.readings.length === 0}
				<p class="text-surface-400 py-8 text-center">No readings assigned yet.</p>
			{:else}
				<div class="space-y-3">
					{#each data.service.readings as reading}
						<div class="border-surface-700 rounded border p-4">
							<div class="flex items-start justify-between">
								<div>
									...
									{#if reading.isOverride}
										<span class="ml-2 rounded bg-yellow-900 px-1.5 py-0.5 text-xs text-yellow-300">Override</span>
									{/if}
									<div class="font-medium">{reading.reference}</div>
									{#if reading.readerFirstName}
										<div class="text-surface-400 mt-1 text-sm">
											Reader: {reading.readerTitle ? reading.readerTitle + ' ' : ''}{reading.readerFirstName} {reading.readerLastName}
										</div>
									{/if}
									{#if reading.notes}
										<div class="text-surface-400 mt-1 text-xs">{reading.notes}</div>
									{/if}
								</div>
								<div class="flex items-center gap-2">
									<button class="text-surface-400 text-xs hover:text-surface-200" onclick={() => editingReadingId = editingReadingId === reading.id ? null : reading.id}>
							...
							{#if editingReadingId === reading.id}
								<form method="POST" action="?/updateReading" class="mt-3 space-y-3 border-t border-surface-700 pt-3">
									<input type="hidden" name="readingId" value={reading.id} />
									<div class="grid grid-cols-1 gap-3 md:grid-cols-2">
										<div>
											<label for="editReadingType-{reading.id}" class="mb-1 block text-xs font-medium">Type</label>
											<select id="editReadingType-{reading.id}" name="readingType" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm">
												{#each Object.entries(ReadingTypeLabels) as [value, label]}
										...
										<div>
											...
											<input type="text" id="editReference-{reading.id}" name="reference" value={reading.reference} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm" />
										</div>
									</div>
									<div class="grid grid-cols-1 gap-3 md:grid-cols-2">
										<div>
											<label for="editReader-{reading.id}" class="mb-1 block text-xs font-medium">Reader</label>
											<select id="editReader-{reading.id}" name="readerId" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm">
												<option value="">Unassigned</option>
												{#each data.people as person}
										...
										<div>
											...
											<input type="text" id="editReadingNotes-{reading.id}" name="notes" value={reading.notes ?? ''} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm" />
										</div>
									</div>
									<button type="submit" class="preset-filled-primary-500 rounded px-4 py-1.5 text-sm">Save</button>
								</form>
	...
	{:else if activeTab === 'music'}
		<div class="max-w-2xl">
			...
			{#if showAddMusic}
				<form method="POST" action="?/addMusic" class="border-surface-700 mb-6 space-y-4 rounded border p-4">
					<h3 class="font-medium">Add Music</h3>
					<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
						<div>
							<label for="newMusicType" class="mb-1 block text-sm font-medium">Type *</label>
							<select id="newMusicType" name="musicType" required class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2">
						...
						<div>
							...
							<select id="newMusicPosition" name="position" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2">
								<option value="">—</option>
					...
					<div>
						...
						<select id="newHymn" name="hymnId" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2">
					...
					<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
						<div>
							...
							<input type="text" id="newMusicTitle" name="title" placeholder="e.g. Anthem title" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2" />
						</div>
						<div>
							<label for="newComposer" class="mb-1 block text-sm font-medium">Composer</label>
							<input type="text" id="newComposer" name="composer" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-2" />
						</div>
					</div>
					<div>
						<label for="newMusicOrder" class="mb-1 block text-sm font-medium">Order</label>
						<input type="number" id="newMusicOrder" name="sortOrder" value={data.service.music.length + 1} class="input w-32 rounded border border-surface-600 bg-surface-800 px-3 py-2" />
					</div>
			...
			{#if data.service.music.length === 0}
				<p class="text-surface-400 py-8 text-center">No music assigned yet.</p>
			{:else}
				<div class="space-y-3">
						...
						<div class="border-surface-700 rounded border p-4">
							<div class="flex items-start justify-between">
								<div>
									<div class="flex items-center gap-2">
										{#if item.position}
											<span class="rounded bg-surface-700 px-2 py-0.5 text-xs text-surface-300">
									...
									{#if item.composer}
										<div class="text-surface-400 text-sm">{item.composer}</div>
									{/if}
								</div>
								<div class="flex items-center gap-2">
									<button class="text-surface-400 text-xs hover:text-surface-200" onclick={() => editingMusicId = editingMusicId === item.id ? null : item.id}>
							...
							{#if editingMusicId === item.id}
								<form method="POST" action="?/updateMusic" class="mt-3 space-y-3 border-t border-surface-700 pt-3">
									<input type="hidden" name="musicId" value={item.id} />
									<div class="grid grid-cols-1 gap-3 md:grid-cols-2">
										<div>
											<label for="editMusicType-{item.id}" class="mb-1 block text-xs font-medium">Type</label>
											<select id="editMusicType-{item.id}" name="musicType" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm">
												{#each Object.entries(MusicTypeLabels) as [value, label]}
										...
										<div>
											...
											<select id="editPosition-{item.id}" name="position" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm">
												<option value="">—</option>
												{#each Object.entries(MusicPositionLabels) as [value, label]}
									...
									<div>
										...
										<select id="editHymn-{item.id}" name="hymnId" class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm">
									...
									<div class="grid grid-cols-1 gap-3 md:grid-cols-2">
										<div>
											...
											<input type="text" id="editMusicTitle-{item.id}" name="title" value={item.title ?? ''} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm" />
										</div>
										<div>
											<label for="editComposer-{item.id}" class="mb-1 block text-xs font-medium">Composer</label>
											<input type="text" id="editComposer-{item.id}" name="composer" value={item.composer ?? ''} class="input w-full rounded border border-surface-600 bg-surface-800 px-3 py-1.5 text-sm" />
										</div>
									</div>
									<button type="submit" class="preset-filled-primary-500 rounded px-4 py-1.5 text-sm">Save</button>
								</form>
